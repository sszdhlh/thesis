...
def plot_lr_scheduler(optimizer, scheduler, epochs=300):
    ...
    plt.ylim(0)
    plt.tight_layout()
    plt.savefig('results/LR.png', dpi=200)
...
def plot_test_txt():  # from utils.utils import *; plot_test()
    ...
    ax.set_aspect('equal')
    plt.savefig('results/hist2d.png', dpi=300)

    fig, ax = plt.subplots(1, 2, figsize=(12, 6), tight_layout=True)
    ax[0].hist(cx, bins=600)
    ax[1].hist(cy, bins=600)
    plt.savefig('results/hist1d.png', dpi=200)
...
def plot_targets_txt():  # from utils.utils import *; plot_targets_txt()
    ...
    for i in range(4):
        ...
        ax[i].legend()
        ax[i].set_title(s[i])
    plt.savefig('results/targets.jpg', dpi=200)
...
def plot_study_txt(f='study.txt', x=None):  # from utils.utils import *; plot_study_txt()
    ...
    plt.savefig('results/study_mAP_latency.png', dpi=300)
...
def plot_labels(labels):
    ...
    plt.savefig('results/labels.png', dpi=200)
    plt.close()
...
def plot_evolution_results(hyp):  # from utils.utils import *; plot_evolution_results(hyp)
    ...
    for i, (k, v) in enumerate(hyp.items()):
        ...
        print('%15s: %.3g' % (k, mu))
    plt.savefig('results/evolve.png', dpi=200)
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=()):  # from utils.utils import *; plot_results()
    ...
    fig.tight_layout()
    ax[1].legend()
    fig.savefig('results/results.png', dpi=200)
